...
function [GAM0,GAM1,PSI,PPI,snames] = gammas_2771_shock_specific_Rules_4(p_st,p_ss,msel,varargin)
redD=msel(22);
nst=msel(10);
if nst>1
    if nargin>3 
...
psi_zlb    = p_st(1,1);
kap        = p_st(2,1);
psipiM      = p_st(3,1); psipiF=p_st(3,2);
psiy       = p_st(4,1);
rhoR       = p_st(5,1);
rhochi     = p_st(6,1);if rhochi~=0;aaaaaaa;end
scaleMPsh  = p_st(7,1);
deltabM     = p_st(8,1); deltabF=p_st(8,2);
deltae     = p_st(9,1);
rhotau     = p_st(10,1);
...
vtau  = ii;ii=ii+1;snames{ii-1}=('\tau');%  [5] Taxes
ve    = ii;ii=ii+1;snames{ii-1}=('tr');%  [6]Total transfers
vQm   = ii;ii=ii+1;snames{ii-1}=('Q');%     [7] Price long term bond
vRLT  = ii;ii=ii+1;snames{ii-1}=('R^L');%     [8] Return long term bond
% Shadow economy
vyc    = ii;ii=ii+1;snames{ii-1}=('y');%  [9] Output
vpic   = ii;ii=ii+1;snames{ii-1}=('\pi');%  [10] Inflation
vRc    = ii;ii=ii+1;snames{ii-1}=('R');%  [11] FFR
vbc    = ii;ii=ii+1;snames{ii-1}=('b');%  [12] Debt

vtauc  = ii;ii=ii+1;snames{ii-1}=('\tau');%  [13] Taxes
vec    = ii;ii=ii+1;snames{ii-1}=('tr');%  [14]Total transfers
vQmc   = ii;ii=ii+1;snames{ii-1}=('Q');%     [15] Price long term bond
vRLTc  = ii;ii=ii+1;snames{ii-1}=('R^L');%     [16] Return long term bond


vg    = ii;ii=ii+1;snames{ii-1}=('g');%  [17] g (transormation of G/Y)
va    = ii;ii=ii+1;snames{ii-1}=('z');%  [18] TFP
vmupR = ii;ii=ii+1;snames{ii-1}=('\mu');% [19] Mark-up shock (rescaled)
vtp   = ii;ii=ii+1;snames{ii-1}=('tp');%   [20]Term premium (res in b eq)
ve_LT = ii;ii=ii+1;snames{ii-1}=('e^L');%   [21]Long term comp of transfers
vynat = ii;ii=ii+1;snames{ii-1}=('y^*');%     [22] Natural output

vdem  = ii;ii=ii+1;snames{ii-1}=('d');%      [23] Demand shock
...
jj=1;
vEy   = ii;ii=ii+1;jj=jj+1;snames{ii-1}=('E(y)'); %   [26] Expected output
vEpi  = ii;ii=ii+1;jj=jj+1;snames{ii-1}=('E(\pi)');%    [27] Expected inflation
vERLT = ii;ii=ii+1;jj=jj+1;snames{ii-1}=('E(R^L)');%    [28] Expected return maturity bond 

vEyc   = ii;ii=ii+1;jj=jj+1;snames{ii-1}=('E(y)'); %   [29] Expected output
vEpic  = ii;ii=ii+1;jj=jj+1;snames{ii-1}=('E(\pi)');%    [30] Expected inflation
vERLTc = ii;ii=ii+1;jj=jj+1;snames{ii-1}=('E(R^L)');%    [32] Expected return maturity bond 
...
GAM0(eq,vpi)     = -psipiF*(1-rhoR);
GAM0(eq,vpic)    = -psipiM*(1-rhoR)+psipiF*(1-rhoR);
...
GAM1(eq,vb)      = deltabF*(1-rhotau);
GAM1(eq,vbc)      = deltabM*(1-rhotau)-deltabF*(1-rhotau);
...
%-----------------------------
% [1] IS Curve
GAM0(eq,vyc)   = 1+Cap_phi_g1;
GAM0(eq,vRc)   = 1-Cap_phi_g1;
GAM0(eq,vg)   = -(1+Cap_phi_g1-rhog);
GAM0(eq,va)   = -(rhozetaA-Cap_phi_g1);
GAM0(eq,vEyc)  = -1;
GAM0(eq,vEpic) = -(1-Cap_phi_g1);
GAM0(eq,vdem) = -(1-Cap_phi_g1)*(1-rhozetaD);

GAM1(eq,vyc)   = Cap_phi_g1;
GAM1(eq,vg)   = -Cap_phi_g1;
...
% [2] Phillips Curve   
varsigbeta_p_1 = 1+varsigma*beta;
GAM0(eq,vyc)    = -(kap/varsigbeta_p_1)*(  1/(1-Cap_phi_g1)+(varphi+alpha)/(1-alpha) );
GAM0(eq,vpic)   = 1;
GAM0(eq,vmupR) = -1;
GAM0(eq,vg)    = kap/((1-Cap_phi_g1)*varsigbeta_p_1);
GAM0(eq,va)    = -kap*Cap_phi_g1/((1-Cap_phi_g1)*varsigbeta_p_1);

GAM0(eq,vEpic)  = -beta/varsigbeta_p_1;

GAM1(eq,vyc)    = -kap*Cap_phi_g1/((1-Cap_phi_g1)*varsigbeta_p_1);
GAM1(eq,vpic)   = varsigma/varsigbeta_p_1;
GAM1(eq,vg)    = kap*Cap_phi_g1/((1-Cap_phi_g1)*varsigbeta_p_1); 
eq=eq+1;
...
% [3] Monetary policy rule 
GAM0(eq,vRc)      = 1;
GAM0(eq,vyc)      = -(psiy+psiDy)*(1-rhoR);
GAM0(eq,va)      = -psiDy*(1-rhoR);
GAM0(eq,vynat)   = psiy*(1-rhoR);   
GAM0(eq,vpic)     = -psipiM*(1-rhoR);

GAM1(eq,vRc)      = rhoR;
GAM1(eq,vyc)      = -psiDy*(1-rhoR);
...
% [4] Tax Rule  
GAM0(eq,vtauc)    = 1;
GAM0(eq,vyc)      = -deltay0*(1-rhotau);
GAM0(eq,vynat)   = deltay0*(1-rhotau);

GAM1(eq,vtauc)    = rhotau;
GAM1(eq,vyc)      = deltay1*(1-rhotau);
GAM1(eq,vynat)   = -deltay1*(1-rhotau);
GAM1(eq,vbc)      = deltabM*(1-rhotau);
GAM0(eq,vec)      = -deltae*(1-rhotau);
GAM0(eq,ve_LT)   = -deltaestar*(1-rhotau);
GAM0(eq,vg)      = -(deltae+deltaestar)*(1-rhotau)/g_st;
PSI(eq,etau)     = 1;
eq=eq+1;
